...
GLfloat deltaTime = 0.0f; //time use to move the camera
...
int main(){
	...
	//glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);  //if run in mac, add this.
...
#pragma endregion ShaderCompile
	...
	unsigned char* image;
	//diffuse map
	image = SOIL_load_image("brickwall.jpg", &width, &height, 0, SOIL_LOAD_RGB);
	...
	//specular map
	image = SOIL_load_image("brickwall_specular.jpg", &width, &height, 0, SOIL_LOAD_RGB);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_NEAREST);
	// Emission map
    image = SOIL_load_image("brickwall_emission.jpg", &width, &height, 0, SOIL_LOAD_RGB);
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		//draw lamp
		objectShader.Use();
		glUniform3f(glGetUniformLocation(objectShader.Program, "light.position"), lightPos.x, lightPos.y, lightPos.z);
